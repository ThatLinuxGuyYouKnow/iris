...
void main() {
  ...
  test(
    'both layers succeed: vision + grounding merged, sources carried',
    () async {
      final r = await merger.compose(
        visionCall: () async =>
            'Stairs 3 metres ahead, door to the left is closed.',
      ...
      expect(r.spokenText, contains(SceneMerger.cameraRaiseCue));
  ...
  test('vision fails -> graph fallback used, grounding still merged', () async {
    final r = await merger.compose(
      ...
      graphFallback:
          'Pre-mapped cue: covered walkway to the library is 20m ahead.',
    );
  ...
  test(
    'both fail and no graph fallback -> human-override floor message',
    () async {
      final r = await merger.compose(
  ...
  test(
    'both fail with graph fallback -> graph cue used, human floor not triggered',
    () async {
      final r = await merger.compose(
        visionCall: () async => throw Exception('offline'),
        groundingCall: () async => throw Exception('offline'),
  ...
  test(
    'vision honestly reports unclear image -> degraded flagged, no fabrication',
    () async {
      final r = await merger.compose(
  ...
  test(
    'timeout on vision is caught and degraded, grounding still used',
    () async {
      final r = await merger.compose(
        visionCall: () =>
  ...
  test('grounding fails silently in speech but flagged degraded', () async {
    ...
    expect(r.spokenText, contains('Path is clear, kerb edge to the right.'));
    expect(r.spokenText, contains(SceneMerger.cameraRaiseCue));
    expect(r.sources, isEmpty);
  ...
  test(
    'grounded vision waits for grounding and avoids raw map duplication',
    () async {
      GroundedAnswer? seenGrounding;

      final r = await merger.compose(
        visionCall: () async => 'ungrounded narration',
        groundedVisionCall: (grounding) async {
          seenGrounding = grounding;
          return 'Kimi narration using the Library context.';
        },
        groundingCall: () async => answer('You are near the Library.'),
      );

      expect(seenGrounding?.text, 'You are near the Library.');
      expect(r.spokenText, contains('Kimi narration'));
      expect(r.spokenText, isNot(contains('You are near the Library.')));
      expect(r.spokenText, contains(SceneMerger.cameraRaiseCue));
      expect(r.layersUsed, containsAll([Layer.vision, Layer.mapsGrounding]));
    },
  );
